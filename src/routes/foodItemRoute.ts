...
const upload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    if(!allowedTypes.includes(file.mimetype)) return cb(new AppError(400, 'Only image files are allowed'))
...
router.post('/custom', isOwner, upload.single('image'), uploadToCloudinary, createCustomFoodItemController)
router.patch('/:itemId/details', isOwner, upload.single('image'), uploadToCloudinary, updateFoodItemDetailValidator, updateFoodItemController);
